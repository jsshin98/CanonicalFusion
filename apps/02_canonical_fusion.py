...
if __name__ == '__main__':
    ...
    data_type = args.dataset.data_type
    ...
    '''
    for d in datasets:
        data_path = os.path.join(data_dir, data_type, color_path, d)
        ...
        for data_name in data_names:
            ...
            if args.canonfusion.use_multi_frame:
                ...
                tmp_img_list, tmp_mask_list, tmp_normal_list = [], [], []
                for view_angle in args.canonfusion.view_angle:
                    img, mask, _ = load_image(os.path.join(data_path, data_name, str(view_angle) + '.png'), pred_res=args.canonfusion.res)
                    if args.canonfusion.use_normal:
                        normal = load_normal(os.path.join(data_path.replace(color_path, normal_path), data_name, str(view_angle) + '.png'), pred_res=args.canonfusion.res)
                    else:
                        normal = np.ones(img.shape)
                    tmp_img_list.append(img)
                    tmp_mask_list.append(mask)
                ...
                input_masks.append(tmp_mask_list)
                input_normals.append(tmp_normal_list)
                smpl_param, smpl_mesh = load_smpl(os.path.join(data_path.replace(color_path, smpl_path), data_name, data_name + '.obj'))

                # smpl_mesh.vertices /= 100

                smpl_params.append(smpl_param)
                smpl_meshes = smpl_mesh
                if data_path.replace(color_path, mesh_path) is not None and args.canonfusion.use_mesh:
