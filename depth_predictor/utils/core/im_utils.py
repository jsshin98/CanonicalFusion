...
def get_plane_params(z, xy, pred_res=512, real_dist=220.0,
                     cut_off=0.2, z_real=False, v_norm=False):
    def gradient_x(img):
    ...
    if z.shape[1] == 1:
        ...
        return plane
        # import cv2
        # cv2.imshow("test", d.detach().cpu().numpy().squeeze().astype(np.uint8))
        # cv2.waitKey(0)
        ...
        normal = [normal_from_grad
                  (grad_x[:, k, :, :].unsqueeze(1),
                   grad_y[:, k, :, :].unsqueeze(1),
                   z[:, k, :, :].unsqueeze(1), res=pred_res) for k in range(z.shape[1])]
