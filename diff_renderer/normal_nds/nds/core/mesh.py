...
class Mesh:
    ...
    Args:
        ...
        colors (tensor) : Color buffer (Vx3)
        uvs (tensor) : UV buffer (Vx2)
        device (torch.device): Device where the mesh buffers are stored
    """

    def __init__(self, vertices, indices, colors, uvs, device='cpu'):
        self.device = device
        ...
        self.uvs = uvs.to(device, dtype=torch.float32) if torch.is_tensor(uvs) else torch.tensor(uvs, dtype=torch.float32, device=device) if uvs is not None else None
    ...
    def to(self, device):
        mesh = Mesh(self.vertices.to(device), self.indices.to(device), self.colors.to(device), self.uvs.to(device), device=device)
    ...
    def detach(self):
        mesh = Mesh(self.vertices.detach(), self.indices.detach(), self.colors.detach(), self.uvs.detach(), device=self.device)
    ...
    def with_vertices(self, vertices):
        ...
        mesh_new = Mesh(vertices, self.indices, self.colors, self.uvs, self.device)
    ...
    def with_colors(self, colors):
        """ Create a mesh with the same connectivity but with different color values

        Args:
            colors (tensor): New color values (Vx3)
        """

        assert len(colors) == len(self.colors)

        mesh_new = Mesh(self.vertices, self.indices, colors, self.uvs, self.device)
        mesh_new._edges = self._edges
        mesh_new._connected_faces = self._connected_faces
        mesh_new._laplacian = self._laplacian
        return mesh_new
