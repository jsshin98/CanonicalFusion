...
from diff_renderer.normal_nds.nds.utils.images import sample
...
def mesh2open3d(mesh, texture=None):
    vertices = mesh.vertices.detach().cpu().numpy() if type(mesh.vertices) == torch.Tensor else mesh.vertices
    faces = mesh.faces.detach().cpu().numpy() if type(mesh.faces) == torch.Tensor else mesh.faces
    ...
    if texture is not None:
        ...
        o3d_mesh.textures = [o3d.io.read_image(texture)]
